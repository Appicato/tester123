...
def SellingMt2Py():
	try:
		...
		if sold == 0:
			...
			if ShopRotePottXXLSlot < 999:
				pottcountvalue = int(player.GetItemCountByVnum(int(RotePottval4)))
				for i in xrange(BuyRedPottsxxl):
					if pottcountvalue >= int(BuyRedPottsxxl * 200):
						break
					net.SendShopBuyPacket(int(ShopRotePottXXLSlot))
					pottcountvalue = pottcountvalue + 200

			if ShopBlauePottKleinSlot < 999:
			...
			if ShopBlauePottMittelSlot < 999:
			...
			if ShopBlauePottXXLSlot < 999:
				pottcountvalue = int(player.GetItemCountByVnum(int(BlauePottval4)))
				for i in xrange(BuyBluePottsxxl):
					if pottcountvalue >= int(BuyBluePottsxxl * 200):
						break
